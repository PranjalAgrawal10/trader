...
The API exchanges the `request_token` at Kite’s token endpoint and stores **encrypted** access (and refresh when present) tokens using ASP.NET Core Data Protection. The OAuth **`state`** parameter is **HMAC-signed** with **`Jwt:Key`** so the browser round-trip still works after a deploy that rotates the Data Protection key ring (persist **`DataProtection__KeyRingPath`** anyway so stored broker tokens and 2FA secrets keep working). **Do not commit API secrets**; keep them in local env files or a secrets manager.
