...
**Troubleshooting — registration / forgot-password email not sent (`5.7.0 Authentication Required`, HTTP 400):** On **DigitalOcean App Platform**, outbound **Gmail SMTP often fails** (blocked ports or auth). Use **SendGrid over HTTPS** instead: create a free [SendGrid](https://sendgrid.com/) account, verify a **single sender**, create an API key, then set **`Smtp__SendGridApiKey`** (Encrypt) and **`Smtp__FromEmail`** (verified address) on the API service. Startup logs show **`Outbound email via SendGrid API`**. Gmail SMTP remains an option for local dev: **`Smtp__IsEnabled=true`**, App password in **`Smtp__Password`**. **`Development`** with no provider logs OTP text to the API console. Also set **`PublicWeb__FrontendBaseUrl`** for registration verify links. **`POST /api/v1/auth/forgot-password`** returns **204** even when no account exists; a **~100 ms** response usually means the email is not registered.
