...
**Authentication:** `POST /api/v1/auth/register` and `POST /api/v1/auth/login` return a JWT when credentials succeed. **`GET /api/v1/auth/me`** (Bearer) returns **`{ "user_id", "email", "role", "created_at" }`** for the signed-in account. If **2FA** is enabled, `POST .../auth/login` returns **200** with `{ "requires_2fa": true, "temp_token": "…" }` (opaque ticket; lifetime **Auth:TwoFactorLoginTicketLifetimeMinutes**, default **30**, clamped **1–120**). Complete sign-in with **`POST /api/v1/2fa/verify-login`** and **`{ "temp_token", "otp" }`** (`otp` accepts a 6-digit TOTP or a single **recovery code**). Repeated failures are rate-limited (**`Auth:MaxFailedTotpAttemptsPerScope`** / **`Auth:TotpAttemptLockoutMinutes`**, defaults **5** / **15 minutes**) per flow (login-step, enrollment confirm, disable-with-code). **Enrollment** (Bearer required, snake_case bodies where noted): `GET /api/v1/2fa/status`, `POST /api/v1/2fa/setup`, `POST /api/v1/2fa/verify-setup` with `{ "otp": "…" }` (returns **`recovery_codes`** once), `POST /api/v1/2fa/cancel-setup`, **`POST /api/v1/2fa/disable`** with **password *or* OTP/recovery** (not both required). TOTP uses **Otp.NET**, **±30s** verification window, secrets at rest **encrypted** via Data Protection (same key ring as broker tokens); use a **persistent** `DataProtection:KeyRingPath` in production so keys and secrets survive redeploys.

The SPA **`/profile`** page shows account fields and **Security** (authenticator setup). **`/security`** redirects to **`/profile`** (query preserved). **2FA is mandatory:** after sign-in, accounts without TOTP enabled are sent to **`/profile?required=1`** and cannot open dashboard, broker, strategies, bots, or trades until setup is complete (`RequiresTwoFactor` gate).
...
2. **Refreshing a deep link (e.g. `/brokers`, `/instruments`) returns 404** — the static host has no file at that path. In the **Static Site** component, under **Custom Pages**, set **Catchall** to **`index.html`** (see [Manage static sites — Custom Pages](https://docs.digitalocean.com/products/app-platform/how-to/manage-static-sites/)). The web build also emits **`404.html`** (copy of `index.html`) for hosts that use a custom error page.
