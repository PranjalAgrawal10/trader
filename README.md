...
- **ML prediction (experimental)**: **`GET /api/v1/predictions/price-direction?instrumentToken=…&interval=…`** (Bearer; same `interval` codes). Uses **ML.NET** (`SdcaLogisticRegression`) trained on-the-fly on recent closes (short-horizon returns + SMA gap). Response: **`direction`** (`up` | `down` | `neutral`), **`confidence`** (0–100), **`modelId`**, **`detail`**. The SPA logs a scrollable **local** history per instrument + interval (large cap; `localStorage`). Not investment advice; heuristic fallback if training fails or output is near 50%.
