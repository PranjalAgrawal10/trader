...
**Troubleshooting — registration / forgot-password email not sent (`5.7.0 Authentication Required`, HTTP 400):** Outbound mail uses **MailKit** + Gmail SMTP. On the **API Web Service** set **`Smtp__IsEnabled=true`**, **`Smtp__Host=smtp.gmail.com`**, **`Smtp__Port=587`**, **`Smtp__EnableTls=true`**, **`Smtp__User`** (full Gmail address), **`Smtp__Password`** (**Google App password**, not your login password — create at [App Passwords](https://myaccount.google.com/apppasswords)), and **`Smtp__FromEmail`** (same address). Also set **`PublicWeb__FrontendBaseUrl`** to your live SPA origin so **registration verify** links are correct. API startup logs **`SMTP enabled for …`** or **`SMTP is enabled but incomplete`** when misconfigured. **`POST /api/v1/auth/forgot-password`** returns **204** even when no account exists (by design); a fast **~100 ms** response usually means the email is not registered — use the exact address from sign-up. Password reset uses a **6-digit email code** (same SMTP settings).
